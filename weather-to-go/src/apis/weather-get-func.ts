...
// 指定した地域の天気情報を取得するAPI
// 取得する情報は、気温・降水確率・風速(毎時)・視程(霧)・標高・1時間あたりの降雪量（ｃｍ）
export const getWeather = async(lat: string, lon: string) => {
    const searchWeatherUrl = `${getUrl}latitude=${lat}&longitude=${lon}&hourly=temperature_2m,precipitation_probability,wind_speed_10m,visibility,weather_code,snowfall&forecast_days=1`
